...
ggplot(mpg, aes(model,manufacturer)) + geom_point() + theme(plot.title = element_text(hjust = 0.5),
...
To make it more interesting, I can put color codes on each point based on the classes variable and add a label using the cty variable.
...
observation_plot <- ggplot(grouped_data,
...
relationship_plot <- ggplot(relationship_cyl_displ,
                            aes(x = Cylinder, y = Engine_Displacement, color = Engine_Displacement))+
                              geom_point(aes(fill = Engine_Displacement))+
